FUNCTION GET_WDB_OBS_SNOW_DEPTH_FOR_RANGE, start_date_YYYYMMDDHH, $
                                           finish_date_YYYYMMDDHH, $
                                           NDV = ndv_, $
                                           STATION_OBJ_ID = station_obj_id, $
                                           SCRATCH_DIR = scratch_dir

; Adapted from GET_WEB_DATA_STATION_SNOW_DEPTH.
...
; Check arguments for correct type and valid contents.
  ...
  save_file_name = 'wdb0_obs_snow_depth_' + $
                   start_date_YYYYMMDDHH + $
                   '_to_' + $
                   finish_date_YYYYMMDDHH + $
                   '.sav'
...
; Determine the number of unique stations reporting snow depth.
  ...
  prev_obj_id = -1L
  obj_id = !NULL
  for rc = 0, num_snow_depth - 1 do begin
...
; Place results in a structure.
  ...
  snow_depth_report_ = REPLICATE({station_obj_id: 0L, $
                                  ...
                                  obs_value_cm: REPLICATE(ndv, num_hours)}, $
                                 num_stations)
  ...
  for rc = 0, num_snow_depth - 1 do begin
      ...
      snow_depth_report_[sc].obs_value_cm[tc] = FLOAT(report[10]) * 100.0
  endfor

  if KEYWORD_SET(station_obj_id) then $
      snow_depth_report_ = snow_depth_report_[0]
