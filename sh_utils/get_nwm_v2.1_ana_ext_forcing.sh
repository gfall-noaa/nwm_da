...
# Fetch National Water Model extended analysis/assim forcing data.

# Greg Fall and Ziya Zhang, 2019-02-21
# 2019-06-19, GF - modified to get NWM v2.0 data from "prod" (operations).

# Example https and ftp locations:
# https://para.nomads.ncep.noaa.gov/pub/data/nccf/com/nwm/para/nwm.20190220/forcing_analysis_assim_extend/
# https://para.nomads.ncep.noaa.gov/pub/data/nccf/com/nwm/para/nwm.20190220/analysis_assim_extend/
# ftp://ftp.ncep.noaa.gov/pub/data/nccf/com/para/com/nwm/para/nwm.20190220/forcing_analysis_assim_extend/
...
DEST_DIR_ROOT=/nwcdev/archive/NWM_v2.1_archive
...
while [ $HOURS_BACK -ge 0 ] ; do
    ...
    # Extended analysis cycles only exist for 16Z.
    if [ "$CYCLE_DATE_HH" != "16" ] ; then
    ...
    # Verify that the "forcing_analysis_assim_extend" subdirectory is
    # present on the server.
    REMOTE_DIR_URL="${URL}/forcing_analysis_assim_extend"
    curl --silent --fail --connect-timeout 60 --max-time 90 -o /dev/null \
         "$REMOTE_DIR_URL"
    STATUS=$?
    if [ $STATUS -ne 0 ] ; then
        if [ $STATUS -ne 22 ] ; then
        ...
        # Check for local forcing_analysis_assim_extend directory.
        DEST_DIR_CHILD=${DEST_DIR_PARENT}/forcing_analysis_assim_extend
        ...
        while [ $TIME_MINUS -ge 0 ] ; do
            ...
            LOCAL_FILE="nwm.${CYCLE_DATE_YYYYMMDD}.t${CYCLE_DATE_HH}z.analysis_assim_extend.forcing.tm${TIME_MINUS_STR}.conus.nc"
            ...
            REMOTE_FILE="nwm.t${CYCLE_DATE_HH}z.analysis_assim_extend.forcing.tm${TIME_MINUS_STR}.conus.nc"
            REMOTE_FILE_URL="${REMOTE_DIR_URL}/${REMOTE_FILE}"
            curl --silent --connect-timeout 60 --max-time 90 --head \
                 "$REMOTE_FILE_URL" | \
