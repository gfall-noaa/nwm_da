...
# Fetch National Water Model extended analysis/assim land model data.
...
while [ $HOURS_BACK -ge 0 ] ; do
    ...
    curl --silent --fail --max-time 60 -o /dev/null "$URL"
    STATUS=$?
    if [ $STATUS -ne 0 ] ; then
    ...
    REMOTE_DIR_URL="${URL}/analysis_assim_extend"
    curl --silent --fail --max-time 60 -o /dev/null "$REMOTE_DIR_URL"
    STATUS=$?
    if [ $STATUS -ne 0 ] ; then
        if [ $STATUS -ne 22 ] ; then
        ...
        DEST_DIR_CHILD=${DEST_DIR_PARENT}/analysis_assim_extend
        ...
        while [ $TIME_MINUS -ge 0 ] ; do
            ...
            LOCAL_FILE="nwm.${CYCLE_DATE_YYYYMMDD}.t${CYCLE_DATE_HH}z.analysis_assim_extend.land.tm${TIME_MINUS_STR}.conus.nc"
            ...
            REMOTE_FILE="nwm.t${CYCLE_DATE_HH}z.analysis_assim_extend.land.tm${TIME_MINUS_STR}.conus.nc"
            REMOTE_FILE_URL="${REMOTE_DIR_URL}/${REMOTE_FILE}"
            curl --silent --max-time 60 --head "$REMOTE_FILE_URL" | \
