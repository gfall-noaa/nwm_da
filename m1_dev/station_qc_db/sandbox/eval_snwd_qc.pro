...
FUNCTION GET_WDB_SNOW_DEPTH_FOR_TARGET, target_obs_date_YYYYMMDDHH, $
...
; to the analysis time.
  ...
  snow_depth_report = TEMPORARY(snow_depth_report_)
...
PRO GET_SNWD_DATA_AND_QC, obs_date_Julian, $
                          prev_hours, $
                          station_obj_id, $
                          qcdb_nc_id, $
                          qcdb_ti, $
  ...
  snwd_sample = $
      GET_WDB_OBS_SNOW_DEPTH_FOR_RANGE(sample_start_YYYYMMDDHH, $
...
PRO SHOW_SNWD_WRIE_DATA, site_str, $
                         time_str, $
                         obs_str, $
                         mdl_str, $
                         obs_date_Julian, $
...
PRO GET_SNWD_PRCP_DATA, obs_date_Julian, $
                        prev_hours_wrie, $
                        station_obj_id, $
                        qcdb_nc_id, $
                        qcdb_ti, $
...
PRO SHOW_SNWD_PRCP_DATA, station_obj_id, $
                         time_str, $
                         obs_str, $
                         mdl_str, $
                         obs_date_Julian, $
...
;         Get snow depth observations.
          wdb_snow_depth_obs = $
              GET_WDB_SNOW_DEPTH_FOR_TARGET(eval_target_YYYYMMDDHH, $
                                            eval_window_start_YYYYMMDDHH, $
                                            eval_window_finish_YYYYMMDDHH, $
                                            min_lon, max_lon, $
                                            min_lat, max_lat)
...
;         Get QC data for snow depth observations.
          NCDF_VARGET, qcdb_nc_id, 'snow_depth_qc', snwd_qc, $
                       COUNT = [num_hrs_pad_prev + num_hrs_pad_post + 1, $
                                qcdb_num_stations], $
                       OFFSET = [qcdb_t1_1, 0]
          NCDF_VARGET, qcdb_nc_id, 'snow_depth_qc_checked', snwd_qc_checked, $
                       COUNT = [num_hrs_pad_prev + num_hrs_pad_post + 1, $
                                qcdb_num_stations], $
                       OFFSET = [qcdb_t1_1, 0]
...
;         difference.
          ...
          wdb_obs_date_Julian = wdb_obs_date_Julian[order]
...
;             Get the station object ID.
              station_obj_id = wdb_snow_depth_obs[wdb_si].station_obj_id

              ;; if (wdb_si eq 126) then begin
              ;;     PRINT, snwd_qc_checked[eval_window_ti, qcdb_si]
              ;;     PRINT, snwd_qc[eval_window_ti, qcdb_si]
              ;;     HELP, wdb_snow_depth_obs[wdb_si], /STRUCT
              ;;     PRINT, abs_diff[wdb_si]
              ;;     STOP
              ;; endif

              if (snwd_qc_checked[eval_window_ti, qcdb_si] eq 0) then CONTINUE

;             Decode QC value.
              qc_str = ''
              qc_val = snwd_qc[eval_window_ti, qcdb_si]
              qc_val_str = STRING(qc_val, FORMAT = '(B)')
              
;             Replace whitespace padding in qc_val_str with zero padding for
;             higher bits.
...
;             Increment the full inventory
              ...
              if (snwd_qc[eval_window_ti, qcdb_si] eq 0) then CONTINUE

;             This observation has been flagged by at least one test. Use
;             abs_diff to decide if this flag is a likely false alarm,
;             possible false alarm, or hit. A "hit" means that the observation
;             has been properly flagged.

;             Judge the QC flag/s based on the difference between the observed
              ...
              case 1 of
                  (abs_diff[wdb_si] ge 0.0) and $
                      ...
                      if (ISA(eval_test_ind) and $
                          ...
                          if (count gt 0) then $
                              snwd_sample_str[ind] = '-'
                          if (sample_qc_hrs gt prev_hours) then $
                              begin
                              PRINT, snwd_sample_str
                              PRINT, obs_str, ' (', $
                                     snwd_sample.station_type + ')'
                  ...
                  (abs_diff[wdb_si] ge abs_diff_thresh_cm[0]) and $
                      ...
                      if (ISA(eval_test_ind) and $
                          (this_inventory[eval_test_ind] eq 1) and $
                          (eval_test_name eq 'precip_consistency')) $
                      then begin
                          ...
                          SHOW_SNWD_PRCP_DATA, station_obj_id, $
                                               time_str, $
                                               obs_str, $
                                               mdl_str, $
                                               obs_date_Julian, $
