...
def parse_args():
    ...
    # min_days_latency is the number of days relative to real time that
    # are updated..........
    parser.add_argument('-m', '--min_days_latency',
                        ...
                        default=0,
                        nargs='?',
                        help='Minimum days of latency for updates.')
    parser.add_argument('-x', '--update_max_hours',
                        type=int,
                        metavar='# of hours',
                        nargs='?',
                        help='Maximum number of hours to update') 
    parser.add_argument('database_path',
                        type=str,
                        metavar='{path to QC database}',
                        help='Full path to QC database')
    ...
    if args.update_max_hours is not None:
        if args.update_max_hours <= 0:
            print('ERROR: --update_max_hours argument must be positive.',
                  format=sys.stderr)
            sys.exit(1)
...
#    var_station_obj_id = qcdb.variables['station_obj_identifier']
    ...
    if obj_id_found is False:
        print('ERROR: No variable in {}'.format(temp_database_path) + 
    ...
    for qcdb_ti in qcdb_update_time_ind:
        ...
        if num_pad_hours > 0 and qcdb_num_stations > 0:
            if args.verbose:
                print('INFO: need to pad previous snow depth QC flag ' +
                      'with {} hours.'.
        ...
        if args.verbose:
            print('Performing snow depth QC for {}'.format(obs_datetime))
                    ...
                    for ind, ts_ind in enumerate(ts_flag_ind):
                        ...
                        if ts_ind_db >= 0:
                            ...
                            if args.check_climatology:
                                ...
                                #       format(station_time_series[ts_ind]))
                                # print('*****  median: {}'.
                                #       format(site_snwd_clim_med_mm * 0.1))
                                # print('***** maximum: {}'.
                                #       format(site_snwd_clim_max_mm * 0.1))
                                # print('*****     iqr: {}'.
                                #       format(site_snwd_clim_iqr_mm * 0.1))
                                # if ref_obs[ind] > \
                                #    site_snwd_clim_max_mm * 0.1:
        ...
        if args.update_max_hours is not None:
            if num_hrs_updated >= args.update_max_hours:
                break
