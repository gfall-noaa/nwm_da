...
def qc_check_snwd_wre(site_snwd_val_cm,
                      site_snwd_obj_id,
                      ...
                      qcdb_snwd_qc_flag):
    '''
    Wrapper function for the snow depth world record exceedance check.
    '''
    ...
    if not qcdb_snwd_qc_chkd[qcdb_si, qcdb_ti] & (1 << qc_bit):
        if qc_durre_snwd_wre(site_snwd_val_cm):
            ...
            qcdb_snwd_qc_flag[qcdb_si, qcdb_ti] = \
                qcdb_snwd_qc_flag[qcdb_si, qcdb_ti] \
                | (1 << qc_bit)
        # Turn on the QC checked bit for this test, regardless of whether the
        ...
        logger.debug('Check "{}" '.format(qc_test_name) +
                     ...
                     'value {} ({})'.format(site_snwd_val_cm, flag_str))
...
def qc_check_snwd_inc_wre(site_snwd_val_cm,
                          site_snwd_obj_id,
                          site_snwd_station_id,
                          qc_test_name,
                          qc_bit,
                          ...
                          flag_snwd_inc_wre_low_val,
                          qcdb_var_time,
                          qcdb_var_time_units):
    '''
    Wrapper function for the world record exceedance check for increase in
    snow depth.
    '''

    logger = logging.getLogger()

    # Check is only performed if preceding snow depth data are available and
    # the test has not been performed yet.
    if (wdb_prev_snwd_si is not None) and \
       (not (qcdb_snwd_qc_chkd[qcdb_si, qcdb_ti] & (1 << qc_bit))):

        # Test has not been performed for this observation, and previous data
        # are available.

        # Note that there are two indices for locating this station:
        # 1. qcdb_si          = the station index in the QC database, which
        #                       applies to:
        #                       qcdb_prev_snwd_qc_flag
        #                       qcdb_snwd_qc_chkd
        #                       qcdb_snwd_qc_flag
        # 2. wdb_prev_snwd_si = the station index in previous snow depth
        #                       observations, which applies to:
        #                       wdb_prev_snwd_val_cm
        ...
        flag, ref_ind = \
            qc_durre_snwd_inc_wre(site_snwd_val_cm,
                                  site_prev_snwd_val_cm,
                                  site_prev_snwd_qc)
        ...
        if flag:
            ...
            if flag_snwd_inc_wre_low_val:

                # Flag previous (low) value as well if that time fits into the
                # database.
...
def qc_check_snwd_streak(site_snwd_val_cm,
                         site_snwd_obj_id,
                         site_snwd_station_id,
                         qc_test_name,
                         qc_bit,
                         ...
                         qcdb_prev_snwd_qc_flag):
    '''
    Wrapper function for the snow depth streak check.
    '''
...
def qc_check_snwd_gap(site_snwd_val_cm,
                      site_snwd_obj_id,
                      site_snwd_station_id,
                      qc_test_name,
                      qc_bit,
                      ...
                      num_hrs_gap,
                      qcdb_prev_snwd_qc_flag,
                      flag_snwd_gap_sec_values,
                      qcdb_var_time,
                      qcdb_var_time_units,
                      check_climatology=True,
                      snwd_clim_med_cm=None,
                      snwd_clim_max_cm=None,
                      snwd_clim_iqr_cm=None):
    '''
    Wrapper function for the snow depth gap check.
    '''
    ...
    if (wdb_prev_snwd_si is not None) and \
       (not (qcdb_snwd_qc_chkd[qcdb_si, qcdb_ti] & (1 << qc_bit))):
        ...
        snwd_ref_ceiling_cm = None
        snwd_ref_defaul_cm = None

        if check_climatology:

            if snwd_clim_med_cm is None or \
               snwd_clim_max_cm is None or \
               snwd_clim_iqr_cm is None:
                logger.warning('Ignoring check_climatology = "{}" '
                               .format(check_climatology) +
                               'because climatology was not provided.')
            else:
                snwd_ref_ceiling_cm = snwd_clim_max_cm + snwd_clim_iqr_cm
                snwd_ref_default_cm = snwd_clim_med_cm

        ts_flag_ind, ref_obs = \
            qc_durre_snwd_gap(site_snwd_val_cm,
                              site_prev_snwd_val_cm,
                              site_prev_snwd_qc,
                              ref_ceiling_cm=snwd_ref_ceiling_cm,
                              ref_default_cm=snwd_ref_default_cm)

        if ts_flag_ind is None:
            logger.error('Snow depth "{}" '.format(qc_test_name) +
                         'check failed ' +
                         'for station {} '.
                         format(site_snwd_station_id) +
                         '({}).'.format(site_snwd_obj_id))
        ...
        for ind, ts_ind in enumerate(ts_flag_ind):
            ...
            if ts_ind_db >= 0:
                ...
                logger.debug('Flagging snow depth data ' +
                             ...
                             'at station {} '.format(site_snwd_station_id) +
                             '({}), '.format(site_snwd_obj_id) +
                             'value {}, '.format(station_time_series[ts_ind]) +
                             'time {}.'.format(flagged_obs_datetime))

                # Make sure the value is not already flagged.
                if qcdb_snwd_qc_flag[qcdb_si, ts_ind_db] & (1 << qc_bit) != 0:
                ...
                if ts_ind_db == qcdb_ti or flag_snwd_gap_sec_values is True:

                    # Turn on the QC bit for this value.
                    qcdb_snwd_qc_flag[qcdb_si, ts_ind_db] = \
                        qcdb_snwd_qc_flag[qcdb_si,ts_ind_db] | (1 << qc_bit)

                    # Identify the value as having been through this check,
                    # even though this is only indirectly the case when
                    # ts_ind_db != qcdb_ti and flag_snwd_gap_sec_values is
                    # True. In that situation, most likely the observation has
                    # already been tested, and passed, but in the current
                    # context it is being flagged.
                    qcdb_snwd_qc_chkd[qcdb_si, ts_ind_db] = \
                        qcdb_snwd_qc_chkd[qcdb_si, ts_ind_db] | (1 << qc_bit)
...
def qc_durre_snwd_inc_tair(site_snwd_val_cm,
                           site_prev_snwd_val_cm,
                           site_prev_snwd_qc,
...
def qc_check_snwd_inc_tair(site_snwd_val_cm,
                           site_snwd_obj_id,
                           site_snwd_station_id,
                           qc_test_name,
                           qc_bit,
                           ...
                           qcdb_snwd_qc_chkd,
                           qcdb_snwd_qc_flag,
                           qcdb_obj_id_var, # only here for error check
                           wdb_prev_snwd_si,
                           wdb_prev_snwd_val_cm,
                           wdb_prev_snwd_obj_id, # only here for error check
                           num_hrs_prev_snwd,
                           wdb_prev_tair_si,
                           wdb_prev_tair_val_deg_c,
                           num_hrs_tair,
                           qcdb_prev_snwd_qc_flag,
                           flag_snwd_inc_tair_low_val,
                           qcdb_var_time,
                           qcdb_var_time_units):
    '''
    Wrapper function for the snow depth increase / temperature consistency
    check.
    '''
    ...
    # preceding air temperature data are avilable, and the test has not been
    # performed yet.
    if (wdb_prev_snwd_si is not None) and \
       (wdb_prev_tair_si is not None) and \
       (not (qcdb_snwd_qc_chkd[qcdb_si, qcdb_ti] & (1 << qc_bit))):

        # Test has not been performed for this observation.

        # Note that there are three indices for locating this station:
        # 1. qcdb_si          = the station index in the QC database, which
        #                       applies to:
        #                       qcdb_prev_snwd_qc_flag
        #                       qcdb_snwd_qc_chkd
        ...
        #                       observations, which applies to:
        #                       wdb_prev_snwd_val_cm
        # 3. wdb_prev_tair_si = the station index in previous air temperature
        #                       observations, which applies to:
        ...
        if len(site_prev_snwd_val_cm) != num_hrs_tair:
            logger.error('(PROGRAMMING) Previous snow depth time index ' + \
                         'is incorrect.')
            qcdb.close()
            sys.exit(1)
        ...
        if len(site_prev_snwd_qc) != num_hrs_tair:
            logger.error('(PROGRAMMING) Previous snow depth QC time ' + \
                         'index is incorrect.')
            qcdb.close()
            sys.exit(1)

        site_prev_tair_val_deg_c = wdb_prev_tair_val_deg_c[wdb_prev_tair_si, :]

        # Programming check. Remove this at some point.
        if len(site_prev_tair_val_deg_c) != num_hrs_tair + 1:
            logger.error('(PROGRAMMING) Previous air temperature time ' + \
                         'index is incorrect.')
            qcdb.close()
            sys.exit(1)

        flag, ref_ind = \
            qc_durre_snwd_inc_tair(site_snwd_val_cm,
                                   site_prev_snwd_val_cm,
                                   site_prev_snwd_qc,
                                   site_prev_tair_val_deg_c)
        ...
        if flag:
            ...
            if flag_snwd_inc_tair_low_val:
                ...
                if ref_ind_db >= 0:
                    ref_datetime = num2date(qcdb_var_time[0] + ref_ind_db,
                                            units=qcdb_var_time_units,
                                            only_use_cftime_datatimes=False)
                    logger.debug('Also flagging low-valued ' +
                                 'observation {} at {}.'.
                                 format(site_prev_snwd_val_cm[ref_ind],
                                        ref_datetime))
...
def qc_durre_snwd_inc_snfl(site_snwd_val_cm,
                           site_prev_snwd_val,
                           site_prev_snwd_qc,
                           site_snfl_val_cm):
    '''
...
def qc_check_snwd_inc_snfl(site_snwd_val_cm,     # scalar
                           site_snwd_obj_id,     # scalar
                           site_snwd_station_id, # scalar
                           qc_test_name,         # scalar
                           qc_bit,               # scalar
                           qcdb,                 # scalar
                           qcdb_si,              # scalar
                           qcdb_ti,              # scalar
                           qcdb_snwd_qc_chkd, # NetCDF [qcdb_si, time]
                           qcdb_snwd_qc_flag, # NetCDF [qcdb_si, time]
                           wdb_prev_snwd_si,     # scalar
                           wdb_prev_snwd_val_cm, # [station, time]
                           num_hrs_prev_snwd,    # scalar
                           wdb_snwd_snfl_si,     # scalar
                           wdb_snwd_snfl_val_cm, # station, time
                           num_hrs_snfl,         # scalar
                           qcdb_prev_snwd_qc_flag, # [qcdb_si, time]
                           flag_snwd_inc_snfl_low_val, # scalar
                           qcdb_var_time,              # [time]
                           qcdb_var_time_units):       # scalar
    '''
    ...
    # performed yet.
    if (wdb_prev_snwd_si is not None) and \
       (wdb_snwd_snfl_si is not None) and \
       (not (qcdb_snwd_qc_chkd[qcdb_si, qcdb_ti] & (1 << qc_bit))):

        # Test has not been performed for this observation.

        # Indices for locating this station:
        # 1. qcdb_si = the station index in the QC database, which will be the
        #              first index in references to:
        #              qcdb_snwd_qc_chkd
        #              qcdb_snwd_qc_flag
        #              qcdb_prev_snwd_qc_flag
        # 2. wdb_prev_snwd_si = the station index in wdb_prev_snwd_val_cm
        # 3. wdb_snwd_snfl_si = the station index in wdb_snwd_snfl_val_cm
        ...
        site_snfl_val_cm = wdb_snwd_snfl_val_cm[wdb_snwd_snfl_si]

        flag, ref_ind = qc_durre_snwd_inc_snfl(site_snwd_val_cm,
                                               site_prev_snwd_val_cm,
                                               site_prev_snwd_qc,
                                               site_snfl_val_cm)
...
def qc_durre_snwd_inc_prcp(site_snwd_val_cm,
                           site_prev_snwd_val,
                           site_prev_snwd_qc,
                           site_prcp_val_mm):
    '''
...
def qc_check_snwd_inc_prcp(site_snwd_val_cm,
                           site_snwd_obj_id,
                           site_snwd_station_id,
                           qc_test_name,
                           qc_bit,
                           ...
                           num_hrs_prcp,
                           qcdb_prev_snwd_qc_flag,
                           flag_snwd_inc_prcp_low_val,
                           qcdb_var_time,
                           qcdb_var_time_units):
    '''
    Wrapper function for the snow depth increase / precipitation consistency
    check.
    '''
    ...
    # Check is only performed if preceding snow depth data are available,
    # corresponding precipitation data are available, and the test has not
    # been performed yet.
    if (wdb_prev_snwd_si is not None) and \
       (wdb_snwd_prcp_si is not None) and \
       (not (qcdb_snwd_qc_chkd[qcdb_si, qcdb_ti] & (1 << qc_bit))):
        ...
        #              qcdb_prev_snwd_qc_flag
        # 2. wdb_prev_snwd_si = the station index in wdb_prev_snwd_val_cm
        # 3. wdb_snwd_prcp_si = the station index in wdb_snwd_prcp_val_mm

        prev_snwd_ti = num_hrs_prev_snwd - num_hrs_prcp # usually 336

        site_prev_snwd_val_cm = \
            wdb_prev_snwd_val_cm[wdb_prev_snwd_si, prev_snwd_ti:]

        site_prev_snwd_qc = \
            wdb_prev_snwd_qc_flag[qcdb_si, prev_snwd_ti:]

        site_prcp_val_mm = wdb_snwd_prcp_val_mm[wdb_snwd_prcp_si]

        flag, ref_ind = \
            qc_durre_snwd_inc_prcp(site_snwd_val_cm,
                                   site_prev_snwd_val_cm,
                                   site_prev_snwd_qc,
                                   site_prcp_val_mm)
        ...
        if flag:
            ...
            if flag_snwd_inc_prcp_low_val:

                # Flag previous value as well if that time fits into the
                # database.
                ref_ind_db = qcdb_ti - num_hrs_prcp + ref_ind
                if ref_ind_db >= 0:
                    ref_datetime =\
...
def qc_durre_snwd_inc_prcp_ratio(site_snwd_val_cm,
                                 site_prev_snwd_val_cm,
                                 prev_snwd_qc,
                                 site_prcp_val_mm):
    '''
...
def qc_check_snwd_inc_prcp_ratio(site_snwd_val_cm,
                                 site_snwd_obj_id,
                                 site_snwd_station_id,
                                 qc_test_name,
                                 qc_bit,
    ...
    '''
    Wrapper function for the snow depth increase / precipitation ratio check.
    '''

    logger = logging.getLogger()

    # Check is only performed in preceding snow depth data are available,
    # corresponding precipitation data are available, and the test has not
    # been performed yet.
...
def qc_durre_snwd_inc_tair_spatial(snwd_val_cm,
                                   prev_snwd_val_cm,
                                   prev_snwd_qc,
                                   nhood_tair_deg_c):
    '''
...
#    var_station_obj_id = qcdb.variables['station_obj_identifier']
    ...
    if (len(qcdb_var_station_obj_id.dimensions) != 1 or
        ...
        logger.error('Database file {} '.format(args.database_path) +
                     '"station_obj_identifier" variable has ' +
    ...
    if obj_id_found is False:
        logger.error('No variable in {}'.format(args.database_path) + 
    ...
    flag_snwd_inc_wre_low_val = False
    flag_snwd_inc_tair_low_val = False # Affects spatial test as well.
    flag_snwd_inc_snfl_low_val = False
    flag_snwd_inc_prcp_low_val = False # Affects ratio test as well.
    ...
    num_flagged_snwd_wre = 0
    num_flagged_snwd_inc_wre = 0
    num_flagged_snwd_streak = 0
    num_flagged_snwd_gap = 0
    num_flagged_snwd_inc_tair = 0
    num_flagged_snwd_inc_snfl = 0
    num_flagged_snwd_inc_prcp = 0
    num_flagged_snwd_inc_prcp_ratio = 0
    ...
    for qcdb_ti in qcdb_update_time_ind:
        ...
        if qcdb_prev_snwd_qc_flag.shape[1] + num_pad_hours != \
           num_hrs_prev_snwd:
            logger.error('(PROGRAMMING) Miscalculation of num_pad_hours.')
            qcdb.close()
            sys.exit(1)
        ...
        logger.debug('Found {} snowfall reports.'.
                     format(wdb_snwd_snfl['num_stations']))
        logger.debug('Query ran in {} seconds.'.
                     format(elapsed_time.total_seconds()))

        # Extract snowfall values and station object identifiers, for
        # convenience (shorter variable names).
        wdb_snwd_snfl_val_cm = wdb_snwd_snfl['values_cm']
        wdb_snwd_snfl_obj_id = wdb_snwd_snfl['station_obj_id']
        ...
        wdb_prev_tair = \
            wdb0.get_air_temp_obs(obs_datetime -
                                  dt.timedelta(hours=num_hrs_tair),
                                  obs_datetime,
                                  scratch_dir=args.pkl_dir,
                                  verbose=args.verbose,
                                  read_pkl=True,
        ...
        if wdb_prev_tair_fetch_datetimes is None:
            wdb_prev_tair_fetch_datetimes = \
                np.array([current_datetime] * (num_hrs_tair + 1))
        ...
        wdb_prev_tair_val_deg_c = wdb_prev_tair['values_deg_c']
        ...
        # Initialize counters for the current time.
        # num_stations_added_this_time = 0
        num_flagged_snwd_wre_this_time = 0
        num_flagged_snwd_inc_wre_this_time = 0
        ...
        num_flagged_snwd_inc_prcp_this_time = 0
        num_flagged_snwd_inc_prcp_ratio_this_time = 0
        ...
        for wdb_snwd_si in range(0, wdb_snwd['num_stations']):
            ...
            if args.check_climatology:
                site_snwd_clim_med_cm = 0.1 * wdb_snwd_clim_med_mm[wdb_snwd_si]
                site_snwd_clim_max_cm = 0.1 * wdb_snwd_clim_max_mm[wdb_snwd_si]
                site_snwd_clim_iqr_cm = 0.1 * wdb_snwd_clim_iqr_mm[wdb_snwd_si]
            else:
                site_snwd_clim_med_cm = None
                site_snwd_clim_max_cm = None
                site_snwd_clim_iqr_cm = None
            ...
            if len(qcdb_si[0]) == 0:
                ...
                if len(wdb_station_meta) != 1:
                    logger.error('Found {} matches in SQL statement ' +
                                 'for station object ID {}; expecting 1.'.
                                 format(len(wdb_station_meta),
                                        site_snwd_obj_id))
                    qcdb.close()
                    sys.exit(1)
            ...
            if len(wdb_prev_snwd_si) != 0:
                ...
                if len(wdb_prev_snwd_si) > 1:
                    logger.error('Multiple matches for station ' +
            ...
            wdb_snwd_snfl_si = []
            for ind, val in enumerate(wdb_snwd_snfl_obj_id):
                if val == site_snwd_obj_id:
                    wdb_snwd_snfl_si.append(ind)

            if len(wdb_snwd_snfl_si) != 0:
                # The object ID for this station was found in the snowfall
                # data.
                if len(wdb_snwd_snfl_si) > 1:
                    logger.error('Multiple matches for station ' +
                                 'object ID {} '.format(site_snwd_obj_id) +
                                 'in snowfall data.')
                    qcdb.close()
                    sys.exit(1)
                wdb_snwd_snfl_si = wdb_snwd_snfl_si[0]
            else:
                wdb_snwd_snfl_si = None
            ...
            qcdb_snwd_qc_chkd, qcdb_snwd_qc_flag = \
                qc_check_snwd_wre(site_snwd_val_cm,
                                  site_snwd_obj_id,
                                  site_snwd_station_id,
                                  qc_test_name,
                                  qc_bit,
            ...
            before = qcdb_snwd_qc_flag[qcdb_si, qcdb_ti] & (1 << qc_bit)
            qcdb_snwd_qc_chkd, qcdb_snwd_qc_flag = \
                qc_check_snwd_inc_wre(site_snwd_val_cm,
                                      site_snwd_obj_id,
                                      site_snwd_station_id,
                                      qc_test_name,
                                      qc_bit,
            ...
            if (qcdb_snwd_qc_flag[qcdb_si, qcdb_ti] & (1 << qc_bit)) and \
                not before:
                num_flagged_snwd_inc_wre_this_time += 1
                num_flagged_snwd_inc_wre += 1
            ...
            before = qcdb_snwd_qc_flag[qcdb_si, qcdb_ti] & (1 << qc_bit)
            qcdb_snwd_qc_chkd, qcdb_snwd_qc_flag = \
                qc_check_snwd_streak(site_snwd_val_cm,
                                     site_snwd_obj_id,
                                     site_snwd_station_id,
                                     qc_test_name,
                                     qc_bit,
                                     ...
                                     num_hrs_streak,
                                     qcdb_prev_snwd_qc_flag)
            if qcdb_snwd_qc_flag[qcdb_si, qcdb_ti] and not before:
                num_flagged_snwd_streak_this_time += 1
                num_flagged_snwd_streak += 1
            ...
            before = qcdb_snwd_qc_flag[qcdb_si, qcdb_ti] & (1 << qc_bit)
            qcdb_snwd_qc_chkd, qcdb_snwd_qc_flag = \
                qc_check_snwd_gap(site_snwd_val_cm,
                                  site_snwd_obj_id,
                                  site_snwd_station_id,
                                  qc_test_name,
                                  qc_bit,
                                  ...
                                  qcdb_var_time,
                                  qcdb_var_time_units,
                                  check_climatology=args.check_climatology,
                                  snwd_clim_med_cm=site_snwd_clim_med_cm,
                                  snwd_clim_max_cm=site_snwd_clim_max_cm,
                                  snwd_clim_iqr_cm=site_snwd_clim_iqr_cm)
            if (qcdb_snwd_qc_flag[qcdb_si, qcdb_ti] & (1<< qc_bit)) and \
               not before:
            ...
            before = qcdb_snwd_qc_flag[qcdb_si, qcdb_ti] & (1 << qc_bit)
            qcdb_snwd_qc_chkd, qcdb_snwd_qc_flag = \
                qc_check_snwd_inc_tair(site_snwd_val_cm,
                                       site_snwd_obj_id,
                                       site_snwd_station_id,
                                       qc_test_name,
                                       qc_bit,
            ...
            before = qcdb_snwd_qc_flag[qcdb_si, qcdb_ti] & (1 << qc_bit)
            qcdb_snwd_qc_chkd, qcdb_snwd_qc_flag = \
                qc_check_snwd_inc_prcp(site_snwd_val_cm,
                                       site_snwd_obj_id,
                                       site_snwd_station_id,
                                       qc_test_name,
                                       qc_bit,
            ...
            if (qcdb_snwd_qc_flag[qcdb_si, qcdb_ti] & (1<< qc_bit)) and \
               not before:
                num_flagged_snwd_inc_prcp_this_time += 1
                num_flagged_snwd_inc_prcp += 1
            ...
            before = qcdb_snwd_qc_flag[qcdb_si, qcdb_ti] & (1 << qc_bit)
            qcdb_snwd_qc_chkd, qcdb_snwd_qc_flag = \
                qc_check_snwd_inc_prcp_ratio(site_snwd_val_cm,
                                             site_snwd_obj_id,
                                             site_snwd_station_id,
                                             qc_test_name,
                                             qc_bit,
            ...
            if (qcdb_snwd_qc_flag[qcdb_si, qcdb_ti] & (1<< qc_bit)) and \
               not before:
                num_flagged_snwd_inc_prcp_ratio_this_time += 1
                num_flagged_snwd_inc_prcp_ratio += 1
            ...
            if wdb_snwd_prcp_si is not None and \
               wdb_prev_snwd_si is not None:
                ...
                if not qcdb_snwd_qc_chkd[qcdb_si, qcdb_ti] & (1 << qc_bit):
                    ...
                    flag, ref_ind = \
                        qc_durre_snwd_inc_prcp_ratio(site_snwd_val_cm,
                                                     site_prev_snwd_val_cm,
                                                     site_prev_snwd_qc,
                                                     site_prcp_val_mm)
            ...
            if wdb_prev_snwd_si is not None and \
               num_prev_tair_neighbors >= min_tair_neighbors:
                ...
                if not qcdb_snwd_qc_chkd[qcdb_si, qcdb_ti] & (1 << qc_bit):
                    ...
                    # current + preceding num_hrs_tair hours.
                    #nhood_tair_min = []
                    ...
                    for nc in range(0, num_prev_tair_neighbors):
                        ...
                        nhood_tair_deg_c[nc,:] = wdb_prev_tair_val_deg_c[ind2,:]

                    flag, ref_ind = \
                        qc_durre_snwd_inc_tair_spatial(site_snwd_val_cm,
                                                   site_prev_snwd_val_cm,
                                                   site_prev_snwd_qc,
                                                   nhood_tair_deg_c)
        ...
        logger.debug('Flagged {} '.format(num_flagged_snwd_wre_this_time) +
                     'snow depth obs. at {} '.format(obs_datetime) +
                     'for world record exceedance.')
        logger.debug('Flagged {} '.
                     format(num_flagged_snwd_inc_wre_this_time) +
                     'snow depth obs. at {} '.format(obs_datetime) +
                     'for world record change exceedance.')
        logger.debug('Flagged {} '.format(num_flagged_snwd_streak_this_time) +
                     'snow depth obs. at {} '.format(obs_datetime) +
                     'for streak.')
        logger.debug('Flagged {} '.format(num_flagged_snwd_gap_this_time) +
                     'snow depth obs. at {} '.format(obs_datetime) +
                     'for gap.')
        logger.debug('Flagged {} '.
                     format(num_flagged_snwd_inc_tair_this_time) +
                     'snow depth obs. at {} '.format(obs_datetime) +
        ...
        logger.debug('Flagged {} '.
                     format(num_flagged_snwd_inc_prcp_this_time) +
                     'snow depth obs. at {} '.format(obs_datetime) +
    ...
    logger.debug('Flagged {} snow depth obs.'.
                 ...
                 'for snow depth/snowfall consistency.')
    logger.debug('Flagged {} snow depth obs.'.
                 format(num_flagged_snwd_inc_prcp) +
                 'for snow depth/precipitation consistency.')
